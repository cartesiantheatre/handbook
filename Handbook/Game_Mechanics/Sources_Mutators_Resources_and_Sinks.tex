...
As mentioned above, resources can be utilized in there raw form or changed through mutators (singular or various) to form a variety of commodities, waste, or new resources, and depending on inputs and processes these will have certain effects, some instant others taken various amounts of time for the effects to be felt. All of the resources, in either their inital or mutated or sunk state can be produced, stored, used, exported, imported, and moved. 

Storage of resources can take place at a variety of instances, initally this may be on a geological/biological or similar level, in its 'natural' state. Storage can also take place after initial mining/harvest at this location, or it may need permanent or correct storage. Resources then go through a lifecycle of consumption depending on city elements such as levels of capitalism, recycling, and consumer use. They may sit on construction sites and supermarket shelves, or go straight into use or for sale at farmers markets. 

How resources are transported is also dependent on a level of infrastructure. Some resources such as apples or grain may be easy to distribute locally in small quantities. Others such as metals, gasses, and large volumes of material require better infrastructure or technology. These may be transports in a variety of manners from foot to interstellar transport depending on a cities agents, technological growth, and other influences. In terms of gameplay, the best outcome is for resources in the natural, transport, and storage states to be displayed visually for the user.

Resources will take various forms and shapes as they are gathered, utilized, combined, or waisted, and may go from being simply terrain to a product to an environmental problem or solution. In this progression and use the game will need to keep track or transforming objects into the various forms, the objects locations, and the effects it will have. For instance, a mineral mined outside of a cities limits will perhaps start as a slight shimmer on a rocky martian landscape, this will then change as a project is set up extracting it from the surface. The minerals new form will depend on how it was extracted but it changes from being X amount of Y to G amount of H. It is still the mineral (Y), but in its mined state (H) however it could also be in its other states, say chemical extraction (I). When in these next state (H)/(I) it is at the location of the mining in one of possibly many "secondary states", it could be loose on the ground due to lack of storage or distaster, it could be in the back of trucks for transport, it could be in a storage facility, or any number of things. The material is then transferred from this state into another state when it reaches another facility or is processed further; in fact it can change any number of times, even being recycled until it is ultimately waisted (sink) or expended (sink), or returned to the earth (sink?), depending on the resource and player choices. 

From this it can be seen that giving resources/sources a large number of options for mutation into other materials requires extensive tracking and detail.  
