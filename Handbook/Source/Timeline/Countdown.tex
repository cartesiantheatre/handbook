...
\StartTimelineDate{A.R.}
Arda receives an asymmetrically encrypted message warning of an allegedly imminent plan drafted by a consortium of Terran financial interests. The information was allegedly all drawn from leaked source material. The anonymous tip-off claims these interests intend to influence the United Nations such that it would see to the organization and landing of two battalions of peacekeepers, MARSBAT1 and MARSBAT2, to Martian theatre. 

The battalions would operate under UNSA's newly formed MARSCOM, which itself operates as an organ of the United Nations, which in turn allegedly answers to the Terran central banking cartels. The field commanders on the ground would be tasked with several prioritized strategic objectives while there. 
...
\startitemize[4]
%\setupwhitespace[big]
\item {\color[colour_item] ALPHA} - KILL OR CAPTURE ATATÜRK\footnote{The NAU CIA attaché to MARSCOM reserve this cryptonym for Arda.}
    ...
    \item IF KILLED, IMPLICATE 'RED REVOLUTIONARIES'. MEDIA PREBRIEFED. BHADRA 2 COMMSAT WILL STANDBY TO BROADCAST NEWS EARTHSIDE.
    \stopitemize
...
\item {\color[colour_item] BRAVO} - CAPTURE ARCADIA
    ...
    \item INSERT PLAINCLOTHES NAU CIA, SPEC FORCE, AND BLACKWATER ASSETS INTO ARCADIA. WILL PROVIDE INTEL.
...
\item {\color[colour_item] CHARLIE} - ASSET EXTRACTION
    \startitemize[n]
    \item ALL NAU CIA ASSETS STILL OPERATING IN THEATRE PRIOR TO KALI'S ARRIVAL MUST BE EXTRACTED AND DEBRIEFED.
    \stopitemize
...
\item {\color[colour_item] DELTA} - ECONOMIC REFORMS
    ...
    \item PROTECT COMBAT ENGINEER'S ERECTION OF 'FIRST CENTRAL BANK OF THE UNITED NATIONS ARCADIAN SETTLEMENT'.
...
\item {\color[colour_item] ECHO} - POLITICAL REFORMS
    ...
    \item RIG ARCADIA HALL ELECTION FOR OOP.\footnote{Office of the President.} FREE ELECTION WITH ALL BALLOT OPTIONS COMPROMISED. ALL CANDIDATES MUST BE SYMPATHETIC TO WHITE LISTED TERRAN INDUSTRIAL AND FINANCIAL INTERESTS.
    \stopitemize
\stopitemize
...
\item {\color[colour_item] FOXTROT} - ASSET INSERTION AND OPERATION
    ...
    \item FIFTH COLUMN CONTRAS WILL CONDUCT INSURGENCY OPERATIONS, EXECUTING FALSE FLAGS MASQUERADING AS 'RED REVOLUTIONARIES'. MUST BE ABLE TO CONDUCT OPERATIONS INDEPENDENTLY W/O NEED FOR COMMUNICATION CONTACT IF BROKEN. PROVIDES MARSCOM W/ CLANDESTINE UNIT BEHIND ARCADIAN DEMARCATION LINE FOLLOWING STRATEGIC OR TACTICAL RETREAT.

    \item IF OBJECTIVE DELTA(THREE) FAILED, FIFTH COLUMN MUST RAISE VITAL FUNDS {\it IN SITU} IN ORDER TO SUPPORT INDEPENDENT ACTIVITIES. FUNDS MUST BE IN LOCAL JENYA CURRENCY. SIERRA'S SEIZED LABORATORY FACILITIES POSSIBLY CONDUCIVE TO {\it IN SITU} SYNTHETIC PRODUCTION NECESSARY FOR NARCOTRAFFICKING. PROVIDES PLAUSIBLE DENIABILITY AND W/O NEED FOR TERRAN IMPORT.
    \stopitemize
\stopitemize
...
\StartTimelineDate{A.R.}
{\it Kali's} commanding officer issues warning orders down the chain of command to prepare for insertion while in Martian areostationary orbit.
\StopTimelineDate
