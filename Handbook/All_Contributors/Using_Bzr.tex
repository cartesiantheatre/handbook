...
%If you have a key already you can utilize this using pageant, otherwise you can use generate a new one using putty.exe...
...
%Pull files in cmd
%Using windows command prompt change directory to appropraite branch using the "cd" command, the exact cd command will depend on where you files have been place or where you choose to download them to. For instance you might use the following code: 
...
%You can then access the files in an up to date manner and make appropriate changes and modifications. Once you are done you will be ready to commit files to the online launchpad repository for Avaneya. To do this in command prompt is simple:
...
%CMD will display its attempts to locate SSH key and appropraite software after an appropraite duratio it will display a revision number, for example 'Pushed up to revision 412.'
...
%An important note is that Pageant is required to be opened and the key located each time you want to pull or push a commit to lp:avaneya. Also don't forgot to login into launchpad using BZR in command prompt. 

%\startCodeExample
%bzr whoami John Smith <smithy@avaneya.com>
%\stopCodeExample

%In addition your first use of BZR will require you to notify BZR and Launchpad of your launchpad ID, BZR and pageant will then link this to your Launchpad credentials contained in your SSH key stored locally and serverside. 
