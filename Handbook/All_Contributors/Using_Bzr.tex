...
%BZR can also be ran in a windows operating system using the command promt or a graphical user interface. The software is available from Canonical, just go to http://bazaar.canonical.com/ and navigate to the downloads section. As Windows does not have a SSH key automatically installed, the software to connect your private key to Avaneya's launchpad repository also requires downloading. Two things are required, the SSH key generation software (if you require new key generation) and the SSH key authentication software, both can be found in the download section at http://www.chiark.greenend.org.uk/~sgtatham/putty/.

%Once at chiark.greenend.org, download the appropriate files for you needs and hardware. The first, putty.exe allows you to create a SSH key by following the steps. Make sure you save the file appropriately and securely and remember your passphrase for future use. Also store your key in a secure place and store a secure backup to prevent the need for repeated continual key generation and pairing. Pageant.exe, the second file, is the software you will requre to open before accessing lp:avaneya under you launchpad name.

%If you have a key already you can utilize this using pageant, otherwise you can use generate a new one using putty.exe and following the steps on screen, before using pageant.

%To utilize your key, open pageant.exe to bring up a window. This will allow you to select the add key button. Using this you can navigate local files to find your saved SSH key which you have either had from previous system/generation or had just recently generated using putty.
...
%BZR in Windows command promt is similar to using BZR in terminal on GNU. Open command promt (search cmd) and type BZR. This will bring up an initial list of commands allowing you to add files, make commits, and more.

%To pull files using command prompt change directory to the appropraite branch using the "cd" command, the exact cd command will depend on where your files have been placed or where you choose to download them to. For instance you might use the following code: 
...
%At this stage the files are committed but they have not been uploaded to the online repository for everyone to access and pull to their own local cache. To upload them you will need to run the bzr command push.

%\startCodeExample
%bzr push lp:avaneya
%\stopCodeExample

%Command prompt will then display its attempts to locate SSH key and appropraite SSH software. After an appropraite duration it will display a revision number, for example 'Pushed up to revision 412.' Remember that pageant.exe is required to be opened with the correct key added to the list and you are required to set your BZR username before using bzr to modify lp:avaneya
...
%To pull files simply click the pull button on top section of buttons. A new window will open, in location type "lp:avaneya" then click ok. You can then make changes to you files locally and browse files in the Bazaar Explorer in a similar manner to the Windows file manager.  
...
%A new window will open. Once again in location type: "lp:avaneya" and then click ok. This will then upload your locally modified files to avaneya's launchpad.

%Remember: pageant and your private SSH key are required to be opened during use of lp:avaneya through BZR.
