...
With these basic building blocks it is possible to have resource processing pipelines as complex or over simplified as a designer likes. Since all of Avaneya's sources, resources, mutators, and sinks are data driven, to add more one simply defines new ones through the Lua interface (see \in{section}[Scripting]). Although there is no theoretical limit to their complexity or the diversity of resources, it is convenient to examine some of a \quotation{core set}.
...
If you look carefully at \in{figure}[figure:Resources_Pipeline] you will notice that there are no sinks. We avoided putting these on the diagram because it would make it harder to read in linking every resource to all possible sinks to cover every conceivable situation. Instead consider \in{figure}[figure:Resource_Sinks] to illustrate some of the possibilities with a reduced resource set.
