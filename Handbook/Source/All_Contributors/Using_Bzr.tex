...
As discussed already in \in{section}[Rationale for Bzr], we opted for Bzr as our revision control management software. We host our master branch on Launchpad. The branch contains the source code to Avaneya, game media, and all relevant documentation - including the book you are reading now. 
...
When saving changes in a branch that was retrieved using the former method, the changes are saved locally only. In the case of a checkout, the changes are saved on the remote project server. We will talk about this further in \in{section}[Committing Changes].
...
If instead you performed a {\tt branch} in \in{section}[Creation of Local Branch], execute the appropriate analogue with a {\tt pull} instead.
