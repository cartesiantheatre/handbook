...
relationship duration + relationship strength ((M)) economic benefit (ratio?) + (cultural ratio)
...
-either money or something else? This needs to be calculated in a raw reward method as a base monetary value is inadequate and will not account for inflation.

income and expenditure -> income/expenditure
current resources & assets liquidated at currnet 'market value'
calculate current purchases.

what is a good or service worth? the simulation will be continually calculating the market value for various goods and services (for instance this will be done when determine if companies enter or exit a market). so whether a unit solely uses money or some other resource, the basic value can be calculated. if the unit uses money to acces resources, the total value of resources can be calculated. if there is a barter system in place either/also 

%
%whats the value of a grain of corn if people are trading either rice or wheat for the corn?
%



%economic benefit ratio
->current cost of living - prospective cost of living 
->factor in extent of change over current level of purchasing, ie the greater the amount the unit can already purchase, the less significant small changes in living situations matter.
