...
% Jenya...
\StartSection{Jenya}
A fiat currency is a currency that has only the endorsement of an authority, such as a government, to back it. It is neither composed of nor redeemable in any material generally perceived of as valuable held in some reserve.

\placefigure
    [left,2*hang]
    {The currency sign of the Arcadian jenya.}
    {\externalfigure[Source/Terminology/Images/Jenya.svg][][height=0.2\textheight]}
{\it Article VII} of Arcadia's {\it Rubicon Act\index{Rubicon Act}} superannuated the Terran \goto{bancor}[Bancor]\index{bancor} fiat currency\index{fiat currency} with the {\it jenya\index{jenya}} (\hbox {\externalfigure[small_jenya]}). It is the first currency to be backed by a predominantly \goto{rhodium}[Rhodium] standard\index{rhodium standard}, with the remainder by other precious metals, such as gold and silver. The jenya became the exclusive legal tender within Arcadia when the Act was passed. This required all Terran interests to acquire Arcadian goods in jenyas only. 

The word {\it jenya} is Sanskrit\index{Sanskrit}. It means {\it of noble origin, genuine, or true wealth}. The idea being that since rhodium is considered precious, indeed, more so than gold on both Earth and Mars, distribution of jenyas across society implied a distribution of real monetary wealth to everyone.
...
Generally the only way of acquiring any kind of high quantity mineral is from high-grade ore. High-grade ore only exists when complex hydrological and volcanic processes have occured which in our solar system has taken place only on Mars and Earth, hence why the Moon is barren. But unlike the Earth, Martian deposits have never been exploited.
