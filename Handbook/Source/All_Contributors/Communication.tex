...
The team uses two primary means of intercommunicating. The first is through the crew mailing list, and the second is over IRC. The Code of Conduct applies to both. See \in{section}[Avaneya Code of Conduct] for more information.
...
\startitemize[4]
\item
{\it Do not ever send email formatted in HTML.} Some mail clients cannot render it. And even if they could, there is no guarantee that it will come out the same. The visually impaired who rely on speech synthesizers can find them unreliable when parsing HTML since it is much more difficult for it to interpret. Others may have to pay for additional bandwidth to retrieve your email from their mail server, since HTML email is larger in size than plain text.\footnote{For more information, consider reading this: \fullahref{http://www1.american.edu/cas/econ/htmlmail.htm}}
