...
Procedural generation is a mathematical technique for automatic generation of data, as opposed to manual implementation or intervention. Think of a computer \quote{understanding} what wood looks like and creating a suitable material for a model on the fly, instead of an artist previously creating the materials and models. Procedural generation can therefore be much faster when generating large repeated (but not necessarily duplicate) objects, such as forests, roads or generic terrain (described below). 
