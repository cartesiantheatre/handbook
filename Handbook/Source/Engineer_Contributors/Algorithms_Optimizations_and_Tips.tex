...
On Intel architectures, we should also remember to check the machine's performance counters\cite[extras={, p. A--13}][intel_optimization_manual] for the frequency of cache misses. This is especially important on consoles.\cite[extras={, p. 568}][game_engine_architecture]

\StartSubSection{Lighting Models}
For lighting, we will have to experiment with the capabilities of OGRE 3D, our rendering engine. Still, there are some algorithms to consider earmarking for the time being. For static objects, such as buildings and terrain, we may be able to take advantage of spherical harmonics for real--time lighting.\cite[extras={, p. 365}][opengl_yellow_book] We can also consider using deferred shading for volume shadows.\cite[extras={, p. 392}][opengl_yellow_book] For global or ambient illumination, we might leverage a hemispherical lighting model. Time will tell, as there is no {\it a priori} substitute for actual experimentation.

\StartSubSection{Memoization}
Memoization, not to be confused with memorization, is a technique in algorithm design that preserves the results of costly computations whenever it is possible to re--use the result without having to perform the entire calculation again for the same input. When scripting, we can take advantage of Lua's built--in {\tt memoization(f)} function whenever possible.\cite[extras={, p. 26}][lua_programming_gems] The function works by creating a new function that returns the same result as {\tt f} on a given input, but by memoizing its result. As long as {\tt f} does not have any side effects, we can use its memoized variant with constant time performance.
...
\placefigure
    ...
    {\externalfigure[Source/Engineer_Contributors/Images/Procedural_Generation_Granite.png][][width=.25\textwidth]}
    {}
    {\externalfigure[Source/Engineer_Contributors/Images/Procedural_Generation_Wood.png][][width=.25\textwidth]}
    {}
    {\externalfigure[Source/Engineer_Contributors/Images/Procedural_Generation_Magma.png][][width=.25\textwidth]}
    {}
    {\externalfigure[Source/Engineer_Contributors/Images/Procedural_Generation_Marble.png][][width=.25\textwidth]}
    {}
    \stopcombination
...
So what about using real topographical data of Mars? Possible, but there is a problem. The best source of topographical data at the time of writing is the {\it Mars Orbiter Laser Altimeter (MOLA)}\index{Mars Orbiter Laser Altimeter (MOLA)} data set.\cite[mola] MOLA was one of the instruments the {\it Mars Global Surveyor}\index{Mars Global Surveyor} orbital spacecraft carried between the years of 1999 to 2001. Unfortunately the spatial resolution is inadequate for our purposes. For every degree of longitude at the planet's equator, there are 128 pixels of sampling available from the cylindrical projection\index{Cylindrical projection} data set. That might seem like a lot, but it is really only about half a kilometre per pixel.
...
Considering this, we selected Makoto Matsumoto's algorithm,\cite[mersenne_twister] a popular high quality pseudo--random number generator. The algorithm offers an excellent balance between performance and quality of output within a very large period.
...
\StartSubSection{Socioeconomic Simulation}
...
Something to consider at least for the time being, however, is the use of a Hubbert curve for modelling resource consumption of finite supplies of natural resources.\cite[hubbert_curve] Whatever models we use, there are some things we need to keep in mind that are fairly fundamental to this project. 
...
Since Avaneya should rely on as much as reasonably possible on actual scientific research to substantiate the quantitative models it uses, {\it The Equality Trust} will be an invaluable repository of research we can continuously mine.\cite[equality_trust]
...
Since we are using the GNU Compiler Collection, we can leverage an already existing implementation of Intel's specification whenever working with {\tt amd64} or {\tt i686} architectures.\footnote{See the {\tt xmmintrin.h} header which ships with the GNU Compiler Collection.} This is especially useful for linear algebra and other math related routines. Better yet, we could instead rely on an architecturally agnostic approach by using GCC's built--in vector extensions.\cite[gcc_vector_instructions]
