...
Eventually when our technology was up and running, we took up correspondence with NASA once again to share what we had come up with. In reflecting on the predicament we initially faced with respect to the original state of the archive and its inaccessibility to the general public, a section manager with the Instrument Software and Science Data Systems Section at the Jet Propulsion Laboratory, wishing to remain anonymous, shared the following remark. \quote{\it You're right that the Viking Lander archive has been largely inaccessible to the general public. Fortunately, we were successful in salvaging the data thru the migration from magnetic tape to CD so that someone, someday could write software to read and use the data. It sounds like you've done that -- I do indeed understand how difficult a job that was}.
